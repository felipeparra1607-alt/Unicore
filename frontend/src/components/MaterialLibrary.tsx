import { ChevronDown, ExternalLink, FileText, MessageCircle, RefreshCw, Upload, X } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import { getDocumentContent, getDocumentFileUrl, getSubjectDocuments, uploadSubjectMaterial, type DocumentContent, type SubjectDocumentsData } from "../api";
...
export default function MaterialLibrary({ subjectId, onAsk }: { subjectId: number; onAsk: (document: Material) => void }) {
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      if (result.duplicate) { setUploadResult(result.message); await load(); return; }
      ...
      for (let attempt = 0; attempt < 180; attempt += 1) {
        ...
        if (latest.processing_status === "ready") { setUploadStep(4); setUploadResult(`Listo · ${latest.title}`); return; }
        if (latest.processing_status === "error") throw new Error(latest.processing_error || "No se pudo preparar el material.");
        await new Promise((resolve) => window.setTimeout(resolve, 1000));
      }
      setUploadResult("El archivo se sigue preparando. Puedes continuar; actualizaremos su estado al recargar.");
    } catch (caught) { setError(`${file.name}: ${caught instanceof Error ? caught.message : "no se pudo preparar el material."}`); }
    finally { setUploading(false); if (inputRef.current) inputRef.current.value = ""; }
  }

  const documents = data?.documents ?? [];
  return <section className={`uc-material-library ${expanded ? "is-expanded" : ""}`}>
    <header className="uc-material-header"><div><p className="uc-eyebrow">Biblioteca de asignatura</p><h2>Materiales <small>{loading ? "" : `${documents.length} archivo${documents.length === 1 ? "" : "s"}`}</small></h2><p>Apuntes y lecturas persistentes, preparados para consultar con UniCore.</p></div><button className="uc-material-toggle" onClick={() => setExpanded((value) => !value)}>{expanded ? "Contraer" : "Ver materiales"}<ChevronDown size={16} /></button></header>
    {expanded && <div className="uc-material-body">
      <div className="uc-material-upload"><input ref={inputRef} type="file" accept=".pdf,.docx,.pptx,.txt,.md" disabled={uploading} onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); }} /><button className="uc-primary-action" disabled={uploading} onClick={() => inputRef.current?.click()}><Upload size={16} /> {uploading ? uploadSteps[uploadStep] : "Subir material"}</button><small>PDF · DOCX · PPTX · TXT · MD · máx. 50 MB</small></div>
      {uploading && <div className="uc-upload-progress"><span style={{ width: `${(uploadStep + 1) * 20}%` }} /><strong>{uploadSteps[uploadStep]}</strong></div>}
