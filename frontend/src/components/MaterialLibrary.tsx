...
export default function MaterialLibrary({ subjectId }: { subjectId: number }) {
  ...
  async function saveClassification() {
    ...
    try { await updateDocumentClassification(editing.id, { material_type: editType, curriculum_unit_id: editUnitId ? Number(editUnitId) : null }); setEditing(null); setNotice("Clasificación actualizada. Actualiza el temario para aplicar cambios estructurales; el texto y los embeddings se conservan."); await load(); }
  ...
  async function confirmDelete() {
    if (!deleting) return;
    setBusy(true); setError(null);
    try { const structural = deleting.material_type === "official_unit"; await deleteSubjectMaterial(deleting.id); if (active?.document.id === deleting.id) setActive(null); setNotice(structural ? "Material eliminado. Actualiza el temario para reconciliar su estructura." : "Material eliminado."); setDeleting(null); await load(); }
