...
export default function StudyPage({ launchContext }: { launchContext?: StudyLaunchContext }) {
  ...
  async function beginSession() {
    if (subjectId == null || !topic.trim() || starting) return;
    setStarting(true); setError(null); setSummary(null);
    try {
      if (mode === "explanation") {
        const result = await startExplanation({ subject_id: subjectId, topic: topic.trim(), document_id: documentId });
        setConversationId(result.conversation_id);
        setMessages([{ role: "assistant", text: result.explanation, sources: result.sources }]);
      } else {
        const result = await startFlashcards({ subject_id: subjectId, topic: topic.trim(), document_id: documentId, item_count: 8 });
  ...
  async function finishSession(modeToSave: keyof typeof modeNames, stats = ratingCounts) {
    ...
    try {
      await createStudySession({
        ...
        topic: topic.trim(), planned_minutes: duration, completed_plan: minutes >= duration,
